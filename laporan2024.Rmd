...
> 1. Proportion
> 2. EDA
> 3. Import detail_lokasi.csv -> tidy
> 4. Lookup institutional geospatial data for each loc in detail_lokasi.csv
...
lokasi_formasi <- read_csv("2024/lokasi_formasi.csv",
                           col_types = cols(lc_code = col_character())) %>% 
  rename(loc_code = lc_code)
...
qs8_skb <- hasil_skb_forward_fill %>% 
...
qs8_filtered_data <- hasil_skb_forward_fill %>% 
  filter(decl_code != "TH") %>% 
  select(skb)

median_skb <- median(qs8_filtered_data$skb)
q3_skb <- quantile(qs8_filtered_data$skb, 3/4)

n_skb_all <- qs8_skb %>% 
  nrow()

qs8_subtitile_plot <- glue("Komparasi skor SKB Umum (n = {scales::number(n_skb_all, big.mark = ',')})")
```
...
qs8_skb %>% 
  ...
  annotate("text", 
           ...
           hjust = 1) +
  # Garis vertikal Q3
  geom_vline(xintercept = skb_q3,
             color = "red") +
  annotate("text",
           x = skb_q3 + 2,
           y = 6,
           label = glue("Q3 SKB\n {scales::number(skb_q3)}"),
           hjust = 0) +
  labs(title = "Distribusi Nilai SKB Umum",
       subtitle = qs8_subtitile_plot,
       x = "SKB",
       y = "Status Kelulusan",
       fill = "n") +
  theme_bw()
...
piv_wider_decl_code <- hasil_skd_na2zero %>% 
...
# Replace NA values dengan 0 (integer)
proporsi_status_kelulusan <- piv_wider_decl_code %>% 
  mutate(across(where(is.numeric), ~replace_na(.x, 0)))

proportions(proporsi_status_kelulusan)

?prop.table

# Inspect NA di decl_code
hasil_skd_na2zero %>% 
  filter(decl_code == is.na(decl_code))

proporsi_status_kelulusan

vis_miss(proporsi_status_kelulusan)
...
### 19. Daerah yang paling banyak diminati

### 20. Daerah yang paling sedikit diminati
